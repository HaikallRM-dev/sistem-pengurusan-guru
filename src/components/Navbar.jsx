...
export default function Navbar({ activeTab, setActiveTab, namaGuru, peranan }) {
  const [settingsBuka, setSettingsBuka] = useState(false);
    ...
    <>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            ...
            <div className="flex items-center gap-3">
              ...
              <button
                onClick={() => setSettingsBuka(true)}
                className="bg-slate-800/60 hover:bg-slate-700 text-slate-200 hover:text-white p-2 rounded-xl border border-slate-700/50 transition"
                title="Tetapan"
              >
      ...
      <div className="md:hidden fixed bottom-0 left-0 right-0 z-50 backdrop-blur-lg bg-slate-900/95 border-t border-slate-800 px-2 py-2">
        <div className="flex justify-around items-center">
          ...
          >
            <div className="p-1 rounded-lg">⚙️</div>
            <span className="text-[10px] tracking-tight">Tetapan</span>
          </button>
        </div>
      </div>

      <SettingsModal buka={settingsBuka} tutup={() => setSettingsBuka(false)} user={user} setActiveTab={setActiveTab} />
    </>
  );
}